...
itemsToIgnore = ['?', '!']
words = [lemmatizer.lemmatize(w.lower()) for w in words if w not in itemsToIgnore]
...
# can make model more efficient too (early stopping?, what else?)
model = Sequential()
...
hist = model.fit(np.array(train_x), np.array(train_y), epochs=100, batch_size=5, callbacks=[tensorboard_callback, earlyStop], verbose=1)
model.save('foodbotModel.h5', hist)
